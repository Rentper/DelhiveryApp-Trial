...
const Order = () => {
  // State to hold order data from the API
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        setOrders(response.data);  // Update the state with the fetched data
      } catch (error) {
  ...
  return (
    <div className="h-[668px] border w-96 m-auto">
      ...
      <div className="mt-3 scroll-auto overflow-y-auto h-[360px]">
        {/* Map through the fetched orders to display them */}
        {orders.length > 0 ? (
          orders.map((order, index) => (
            <div
              key={index}
              className="border mt-2 p-0.5 shadow-[0_0px_5px_rgba(0,0,0,0.25)] rounded-md m-auto text-xs w-[370px] flex h-[70px]"
            >
              <div className="w-64">
                <p className="text-gray-400 text-xs pl-1">Location</p>
                <p className="text-base mt-2 pl-1">{order.location || "Nanakhed, Ujjain"}</p>
              </div>
              <div>
                <p className="ml-4">|</p>
                <p className="text-[10px]">{order.distance || "250km"}</p>
              ...
              <div className="w-64">
                ...
                <p className="text-base mt-2">{order.destination || "Freeganj Ujjain"}</p>
              </div>
            </div>
          ))
        ) : (
